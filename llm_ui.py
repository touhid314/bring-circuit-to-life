...
def process_prompt(prompt:str, llm_model, ckt_netlist, analyzer):
    ...
    '''
    # TODO: use llama-cpp-agent or easy-llama
    import re
    gen_plt = None
    gen_axe = None

    response = get_llm_response(prompt, llm_model,context=None, ckt_netlist=ckt_netlist, show_execution_time=True)
    ...
    if len(exec_commands) != 0:
        ...
        for exec_cmd in exec_commands:
            try:
                ...
                # handle the outputs of the simulation (result variable)
                ...
                # ## generate comment by the llm on the simulation output 
                # if isinstance(result, dict) and 'sim_descr' in result:
                #     sim_descr = result["sim_descr"]
                # else:
                #     sim_descr = None

                # response2 = get_llm_response(prompt, llm_model, context=sim_descr, ckt_netlist=ckt_netlist, show_execution_time=True) # TODO: use modified ckt_netlist here

                # non_exec_response = non_exec_response + response2
